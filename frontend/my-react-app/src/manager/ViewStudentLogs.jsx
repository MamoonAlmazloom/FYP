  ...
  useEffect(() => {
    const loadStudentLogs = async () => {
      try {
        ...
        try {
          const response = await ManagerAPI.getStudentLogs(
            managerId,
            studentId
          );
          if (response.success) {
            setLogs(response.logs || []);
            console.log("Student logs loaded:", response.logs);
          } else {
            console.error("Failed to load student logs:", response.error);
            setError(response.error || "Failed to load student logs");
          }
        } catch (apiError) {
          console.error("API Error loading student logs:", apiError);
