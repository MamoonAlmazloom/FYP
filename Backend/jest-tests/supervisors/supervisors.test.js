...
jest.mock('../../models/supervisorModel', () => ({
  getAssignedStudentsBySupervisorId: jest.fn(),
}));
  ...
  describe('GET /api/supervisors/my/students (Supervisor View Current Students)', () => {
    it('should allow a supervisor to view their list of currently supervised students', async () => {
      const mockStudentList = [
        { student_id: 'student1', name: 'Alice Wonderland', project_title: 'AI Adventures' },
        { student_id: 'student2', name: 'Bob The Builder', project_title: 'Constructing Test Cases' },
      ];
      supervisorModel.getAssignedStudentsBySupervisorId.mockResolvedValue(mockStudentList);
      ...
      expect(supervisorModel.getAssignedStudentsBySupervisorId).toHaveBeenCalledWith(supervisorId);
    });

    it('should return an empty list if supervisor has no current students', async () => {
      supervisorModel.getAssignedStudentsBySupervisorId.mockResolvedValue([]);
      const response = await request(app)
