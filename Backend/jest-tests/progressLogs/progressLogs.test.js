...
describe("Progress Log Endpoints", () => {
  ...
  describe("POST /api/projects/:projectId/logs (Submit Progress Log)", () => {
    ...
    it("should allow a student to submit a progress log", async () => {
      ...
      progressLogModel.createLog.mockResolvedValue({
        id: logId,
      ...
      expect(progressLogModel.createLog).toHaveBeenCalledWith(
  ...
  describe("PUT /api/progress-logs/:logId/review (Supervisor Review Log)", () => {
    const feedbackData = {
      feedback: "Good details. Keep it up.",
      is_signed: true,
    };
    ...
    it("should allow assigned supervisor to add feedback and sign a log", async () => {
      ...
      progressLogModel.addFeedbackToLog.mockResolvedValue({
        ...mockLog,
        ...feedbackData,
      });
      ...
      expect(progressLogModel.addFeedbackToLog).toHaveBeenCalledWith(
        logId,
        supervisorId,
        feedbackData.feedback,
        feedbackData.is_signed
      );
      expect(notificationService.notifyUser).toHaveBeenCalledWith(
        studentId,
        expect.stringContaining("feedback on your progress log"),
        expect.any(String)
      );
    });
  ...
  describe("GET /api/admin/students/:studentId/progress-logs (Manager View Student Logs)", () => {
    ...
    it("should allow a manager to view progress logs for any student", async () => {
      ...
      expect(progressLogModel.getLogsByStudentId).toHaveBeenCalledWith(
        targetStudentId
      );
    });
