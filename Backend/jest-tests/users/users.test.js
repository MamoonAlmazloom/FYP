...
describe('User Endpoints (F1: User Account Management)', () => {
  ...
  describe('POST /api/users/register (TC-B1.3)', () => {
    ...
    it('should register a new student successfully', async () => {
      ...
      expect(authModel.findUserByEmail).toHaveBeenCalledWith(validStudentData.email);
    ...
    it('should return 409 if email already exists', async () => {
      authModel.findUserByEmail.mockResolvedValue({ id: 'existingUser', email: validStudentData.email }); // Email is taken
    ...
    /*
    it('should return 409 if student_id_number already exists', async () => {
      authModel.findUserByEmail.mockResolvedValue(null); // Email is fine
      // Assume another model/method checks for student_id_number uniqueness
      const studentModel = require('../../models/studentModel'); // hypothetical
      jest.mock('../../models/studentModel', () => ({ findStudentByStudentIdNumber: jest.fn() }));
