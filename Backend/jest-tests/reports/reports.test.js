...
describe('Report Endpoints', () => { // Main describe block can be more generic
  ...
  describe('POST /api/projects/:projectId/reports (Submit Report)', () => {
    ...
    const mockProjectForReport = { id: projectId, student_id: studentId, supervisor_id: 'supervisorOfProject123' };
    ...
    it('should allow a student to submit a report with a file and notify supervisor', async () => {
      ...
      expect(reportModel.createReport).toHaveBeenCalledWith(expect.objectContaining({
        project_id: projectId,
        student_id: studentId,
      ...
      const { notifyUser } = require('../../services/notificationService');
       expect(notifyUser).toHaveBeenCalledWith(mockProjectForReport.supervisor_id, expect.stringContaining('New report submitted'), expect.objectContaining({ reportId: reportId, projectId: projectId }));
    });
  ...
  describe('PUT /api/reports/:reportId/review (Supervisor Review Report)', () => {
    const feedbackData = { feedback: "Good progress, but need more details on methodology.", grade: "B+" };
    const mockReport = { id: reportId, project_id: projectId, student_id: studentId, supervisor_id: supervisorId };
    ...
    it('should allow assigned supervisor to add feedback and grade to a report', async () => {
      ...
      reportModel.addFeedbackToReport.mockResolvedValue({ ...mockReport, ...feedbackData });
      ...
      // expect(projectModel.findProjectById).toHaveBeenCalledWith(projectId);
      expect(reportModel.addFeedbackToReport).toHaveBeenCalledWith(reportId, supervisorId, feedbackData.feedback, feedbackData.grade);
      expect(notificationService.notifyUser).toHaveBeenCalledWith(studentId, expect.stringContaining("feedback on your report"), expect.any(String));
    });

    it('should return 403 if supervisor is not assigned to the project of the report', async () => {
      reportModel.findReportById.mockResolvedValue(mockReport);
      projectModel.findProjectById.mockResolvedValue({ id: projectId, supervisor_id: 'otherSupervisor', student_id: studentId }); // Different supervisor
  ...
  describe('GET /api/reports/:reportId (Supervisor View Specific Report)', () => {
    const pastReport = { id: reportId, project_id: 'pastProject1', title: 'Final Report 2022', student_id: 'pastStudent', supervisor_id: supervisorId, file_path: 'archive/report.pdf' };
    ...
    it('should allow supervisor to view a specific report from their past projects', async () => {
      ...
      expect(reportModel.getReportByIdForSupervisor).toHaveBeenCalledWith(reportId, supervisorId);
    });
  ...
  describe('GET /api/admin/students/:studentId/reports (Manager View Student Reports)', () => {
    ...
    const mockReportsForStudent = [
      { id: 'rep1', student_id: targetStudentId, title: 'Report Alpha', file_path: 'uploads/alpha.pdf' },
      { id: 'rep2', student_id: targetStudentId, title: 'Report Beta', file_path: 'uploads/beta.pdf' },
    ];
    ...
    it('should allow a manager to view reports for any student', async () => {
      reportModel.getReportsByStudentId.mockResolvedValue(mockReportsForStudent);
      ...
      expect(reportModel.getReportsByStudentId).toHaveBeenCalledWith(targetStudentId);
    });
