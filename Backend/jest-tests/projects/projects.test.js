...
describe("Project Endpoints", () => {
  ...
  describe("POST /api/projects (Supervisor Propose New Project)", () => {
    ...
    it("should allow a supervisor to propose a new project", async () => {
      projectModel.createProject.mockResolvedValue({
        id: projectId,
  ...
  describe("POST /api/projects/available/:projectId/select (Student Select Project)", () => {
    it("should allow a student to select an available project", async () => {
      ...
      projectModel.getProjectsByStudent.mockResolvedValue([]); // No existing projects
      ...
      projectModel.assignProjectToStudent.mockResolvedValue({
        success: true,
        project: { id: availableProjectId, student_ids: [studentId] },
      }); // Simplified
      ...
      expect(projectModel.assignProjectToStudent).toHaveBeenCalledWith(
        availableProjectId,
        studentId
      );
    });
    ...
    it("should return 400 if project is not available or at capacity", async () => {
      ...
      expect(response.body.message).toMatch(
        /project is not available or at full capacity/i
      );
    });

    it("should return 400 if student already has a project (if limit is 1)", async () => {
      projectModel.getProjectsByStudent.mockResolvedValue([
        { id: "existingProj1", student_id: studentId },
      ...
      expect(response.statusCode).toBe(400);
      expect(response.body.message).toMatch(
        /student already has an active project/i
      );
    });
  ...
  describe("GET /api/projects/archive (Supervisor Access Past Projects)", () => {
    ...
    it("should allow a supervisor to view their archived projects", async () => {
      ...
      expect(projectModel.getProjectsBySupervisor).toHaveBeenCalledWith({
        supervisor_id: supervisorId,
        status: "Archived",
      });
    });
  ...
  describe("Manager Project Endpoints (F5)", () => {
    ...
    describe("POST /api/projects/:projectId/assign-examiner", () => {
      it("should allow a manager to assign an examiner to a project", async () => {
        projectModel.findProjectById.mockResolvedValue({
          id: targetProjectId,
          status: "Approved",
        }); // Project exists and is in a state to assign examiner
        projectModel.assignExaminerToProject.mockResolvedValue({
          success: true,
          project: { id: targetProjectId, examiner_id: examinerUserId },
        });
        ...
        expect(projectModel.assignExaminerToProject).toHaveBeenCalledWith(
          targetProjectId,
          examinerUserId
        );
      });
      ...
      it("should return 400 if examiner user is invalid or project not in assignable state", async () => {
        ...
        expect(response.body.message).toMatch(
          /project not in assignable state/i
        );
        ...
        projectModel.assignExaminerToProject.mockRejectedValue(
          new Error("Invalid examiner or role")
    ...
    describe("POST /api/projects/:projectId/assign-moderator", () => {
      it("should allow a manager to assign a moderator to a project", async () => {
        ...
        projectModel.assignModeratorToProject.mockResolvedValue({
          success: true,
          project: { id: targetProjectId, moderator_id: moderatorUserId },
        });
        ...
        expect(projectModel.assignModeratorToProject).toHaveBeenCalledWith(
          targetProjectId,
          moderatorUserId
        );
      });
    ...
    describe("GET /api/projects?status=Approved (View Approved Projects)", () => {
      it("should allow a manager to view all approved projects", async () => {
        const approvedProjects = [
          { id: "proj1", status: "Approved" },
          { id: "proj2", status: "Approved" },
        ];
        ...
        expect(projectModel.getProjectsByStatus).toHaveBeenCalledWith(
          "Approved"
        );
      });
    });
    ...
    describe("POST /api/projects/:projectId/archive (Archive Project)", () => {
      it("should allow a manager to archive an approved project", async () => {
        ...
        projectModel.updateProjectStatus.mockResolvedValue({
          success: true,
          project: { id: targetProjectId, status: "Archived" },
        });
        ...
        expect(projectModel.updateProjectStatus).toHaveBeenCalledWith(
          targetProjectId,
          "Archived"
        );
      });
