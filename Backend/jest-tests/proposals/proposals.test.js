...
jest.mock('../../models/proposalModel', () => ({
...
jest.mock('../../services/notificationService', () => ({ // Assuming you have such a service
  notifyUser: jest.fn(),
  notifyRole: jest.fn(),
}));
...
describe('Proposal Endpoints (F2: Proposal Management)', () => {
  ...
  beforeAll(() => {
    ...
    moderatorToken = global.getModeratorToken ? global.getModeratorToken({ userId: moderatorId }) : 'dummyModeratorToken';
  });
  ...
  describe('POST /api/proposals (Submit Proposal)', () => {
    const proposalData = { title: 'AI for Climate Change', description: 'Using AI to model climate patterns.', objectives: 'Objective 1' };

    it('should allow a student to submit a proposal successfully', async () => {
      proposalModel.createProposal.mockResolvedValue({ id: proposalId, student_id: studentId, status: 'Pending', ...proposalData });
      ...
      expect(proposalModel.createProposal).toHaveBeenCalledWith(expect.objectContaining({ ...proposalData, student_id: studentId }));
      // TC-B4.3: Supervisor Notification for New Proposal
      const { notifyRole } = require('../../services/notificationService');
      expect(notifyRole).toHaveBeenCalledWith('supervisor', expect.stringContaining('New proposal submitted'), expect.objectContaining({ proposalId: proposalId }));
    });
  ...
  describe('PUT /api/proposals/:proposalId/review (Supervisor Review)', () => {
    const reviewData = { status: 'ApprovedBySupervisor', feedback: 'Good proposal.' };
    const mockProposal = { id: proposalId, student_id: 'anotherStudentId', supervisor_id: supervisorId, status: 'Pending' };
    ...
    it('should allow an assigned supervisor to approve a proposal', async () => {
      ...
      proposalModel.updateProposalStatus.mockResolvedValue({ ...mockProposal, ...reviewData });
      ...
      expect(proposalModel.updateProposalStatus).toHaveBeenCalledWith(proposalId, reviewData.status, reviewData.feedback, supervisorId);
      ...
      expect(notifyUser).toHaveBeenCalledWith(mockProposal.student_id, expect.stringContaining('proposal has been reviewed by your supervisor'), expect.objectContaining({ proposalId: proposalId }));
      if (reviewData.status === 'ApprovedBySupervisor') {
        expect(notifyRole).toHaveBeenCalledWith('moderator', expect.stringContaining('Proposal requires moderation'), expect.objectContaining({ proposalId: proposalId }));
      }
    });
    ...
    it('should return 403 if supervisor is not assigned to the proposal (or student)', async () => {
      ...
      proposalModel.findProposalById.mockResolvedValue({ ...mockProposal, supervisor_id: 'otherSupervisorId' });
       const response = await request(app)
        .put(`/api/proposals/${proposalId}/review`)
    ...
    it('should return 400 for invalid status transition', async () => {
      proposalModel.findProposalById.mockResolvedValue({ ...mockProposal, status: 'ApprovedByModerator' }); // Already past supervisor approval
      const response = await request(app)
        .put(`/api/proposals/${proposalId}/review`)
        .set('Authorization', `Bearer ${supervisorToken}`)
        .send(reviewData);
  ...
  describe('PUT /api/proposals/:proposalId/moderate (Moderator Action)', () => {
    const moderationData = { status: 'ApprovedByModerator', feedback: 'Final approval.' };
    const mockProposalForModeration = { id: proposalId, student_id: 'studentX', supervisor_id: 'supervisorY', status: 'ApprovedBySupervisor' };

    it('should allow a moderator to approve a proposal', async () => {
      proposalModel.findProposalById.mockResolvedValue(mockProposalForModeration);
      proposalModel.updateProposalStatus.mockResolvedValue({ ...mockProposalForModeration, ...moderationData });
      ...
      expect(proposalModel.updateProposalStatus).toHaveBeenCalledWith(proposalId, moderationData.status, moderationData.feedback, moderatorId);

      // TC-B4.1: Proposal Decision Notification (Student and Supervisor)
      const { notifyUser } = require('../../services/notificationService');
      expect(notifyUser).toHaveBeenCalledWith(mockProposalForModeration.student_id, expect.stringContaining('proposal has been moderated'), expect.objectContaining({ proposalId: proposalId }));
      expect(notifyUser).toHaveBeenCalledWith(mockProposalForModeration.supervisor_id, expect.stringContaining('proposal for your student has been moderated'), expect.objectContaining({ proposalId: proposalId }));
    });

    it('should return 400 if proposal is not in a state for moderation', async () => {
      proposalModel.findProposalById.mockResolvedValue({ ...mockProposalForModeration, status: 'Pending' }); // Not yet approved by supervisor
      const response = await request(app)
        .put(`/api/proposals/${proposalId}/moderate`)
        .set('Authorization', `Bearer ${moderatorToken}`)
        .send(moderationData);
      expect(response.statusCode).toBe(400);
      expect(response.body.message).toMatch(/proposal not ready for moderation/i);
    });
  });
  ...
  describe('GET /api/proposals/my (Student View Own Proposals)', () => {
    it('should allow a student to view their own proposals', async () => {
      const studentProposals = [{ id: 'prop1', student_id: studentId, title: 'My First Proposal', status: 'Pending' }];
      ...
      expect(proposalModel.getProposalsByStudentId).toHaveBeenCalledWith(studentId);
    });
  ...
  describe('GET /api/proposals/:proposalId (Moderator View Specific Proposal)', () => {
    it('should allow a moderator to view any proposal details', async () => {
      const specificProposal = { id: proposalId, title: 'Test Prop', status: 'ApprovedBySupervisor', student_id: 'anyStudent' };
  ...
  describe('PUT /api/proposals/:proposalId (Student Modify Proposal)', () => {
    ...
    const modifiableProposal = { id: proposalId, student_id: studentId, status: 'NeedsModificationBySupervisor' };
    ...
    it('should allow student to modify their proposal if in modifiable state', async () => {
      ...
      proposalModel.updateProposalDetails.mockResolvedValue({ ...modifiableProposal, ...updatedData, status: 'Pending' }); // Status might reset
      ...
      expect(proposalModel.updateProposalDetails).toHaveBeenCalledWith(proposalId, studentId, updatedData);
    });

    it('should return 403 if student tries to modify another student proposal', async () => {
      proposalModel.findProposalById.mockResolvedValue({ ...modifiableProposal, student_id: 'otherStudent' });
      const response = await request(app)
        .put(`/api/proposals/${proposalId}`)
        .set('Authorization', `Bearer ${studentToken}`)
        .send(updatedData);
    ...
    it('should return 400 if proposal is not in a modifiable state', async () => {
      proposalModel.findProposalById.mockResolvedValue({ ...modifiableProposal, status: 'ApprovedBySupervisor' });
      const response = await request(app)
        .put(`/api/proposals/${proposalId}`)
        .set('Authorization', `Bearer ${studentToken}`)
        .send(updatedData);
      expect(response.statusCode).toBe(400);
      expect(response.body.message).toMatch(/cannot be modified in its current state/i);
    });
  });
  ...
  describe('POST /api/proposals/:proposalId/comments (Moderator Add Comment)', () => {
    ...
    const proposalToComment = { id: proposalId, status: 'ApprovedBySupervisor' };
    ...
    it('should allow a moderator to add a comment to a proposal', async () => {
      ...
      proposalModel.addCommentToProposal.mockResolvedValue({ id: 'comment123', proposal_id: proposalId, user_id: moderatorId, text: commentData.text });
      ...
      expect(proposalModel.addCommentToProposal).toHaveBeenCalledWith(proposalId, moderatorId, commentData.text);
    });
